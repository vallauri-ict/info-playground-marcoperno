...
const campiPrenotazione = ["id", "cliente_persona_id", "data", "negozio_id", "ora", "servita", "lista", "volontario_persona_id"]
        ...
        if (controlloCitta != -1) {
            ...
            if (ris.id != '-1' && ris.tipo == 'cliente') {
                ...
                if (ris.id != '-1' && ris.tipo == 'volontario') {
                    ...
                    let query = "INSERT INTO prenotazione (cliente_persona_id, negozio_id, data, ora, volontario_persona_id, lista, servita)" +
                        " VALUES (" + con.escape(obj["cliente_persona_id"]) + ", " + con.escape(obj["negozio_id"]) + "," +
                        " " + con.escape(obj["data"]) + ", " + con.escape(obj["ora"]) + "," +
                        " " + con.escape(obj["volontario_persona_id"]) + ",  " + con.escape(obj["lista"]) + ", '0'); ";
...
async function personaStore(req, res) {
    try {
        ...
        if (ris.id != '-1') {
            ...
            let query = "INSERT INTO persona ( nome, cognome, indirizzo, tipopersona_id, email, pwd)"+
            " VALUES ( "+ con.escape(obj["nome"])+", "+ con.escape(obj["cognome"])+","+
            " "+ con.escape(obj["indirizzo"])+", "+ con.escape(obj["tipopersona_id"])+","+
            " "+ con.escape(obj["email"])+", ";
            
            if(ris.tipo == "cliente")
                query+="NULL);";
            else
                query+=con.escape(obj["pwd"])+");";
