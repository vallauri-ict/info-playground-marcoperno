...
function _getNomeCognome(request, response) {
    ...
    if(_controllaSession(parametri.Username)==true)
    {
        ...
        con.query("SELECT Nome,Cognome FROM utenti WHERE Username like "+con.escape(parametri.Username), function (err, result, fields) {
            ...
            obj.Cognome=result[0].Cognome;
            let x = JSON.stringify(obj);
            disp.inviaRisposta(response, 1, x);
...
function _verificaLogin(request, response) {
    let datiPacchetto=[];
    request.on('data', function(dato){
        datiPacchetto+=dato;
    })
    ...
    request.on("end", function(){
        ...
        if(_controllaSession(parametri.Username)==true)
        {
            disp.inviaRisposta(response, 1, "Già loggato");
        }
        else
        {
            disp.inviaRisposta(response, 0, " no Già loggato");
        }
    });
}
...
module.exports.verificaLogin = _verificaLogin;